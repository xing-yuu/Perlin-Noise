...
public:
    ...
    float eval4(float x,float y)
    {
        ...
        float xt = xyOffset + cos(s * 2.0f * PI) * dx / (2.0f * PI);
        float yt = xyOffset + cos(t * 2.0f * PI) * dy / (2.0f * PI);
        float zt = xyOffset + sin(s * 2.0f * PI) * dx / (2.0f * PI);
        float wt = xyOffset + sin(t * 2.0f * PI) * dy / (2.0f * PI);
 ...
 //       cout << "(" << xt << "," << yt << "," << zt << ")" << endl;
     ...
     //   cout << nowseed << endl;
        ...
        float nx000 = lerpp(c0000, c0001, sx);
        float nx001 = lerpp(c0010, c0011, sx);
        float nx010 = lerpp(c0100, c0101, sx);
        float nx011 = lerpp(c0110, c0111, sx);
        float nx100 = lerpp(c1000, c1001, sx);
        float nx101 = lerpp(c1010, c1011, sx);
        float nx110 = lerpp(c1100, c1101, sx);
        float nx111 = lerpp(c1110, c1111, sx);

        float ny00 = lerpp(nx000, nx001, sy);
        float ny01 = lerpp(nx010, nx011, sy);
        float ny10 = lerpp(nx100, nx101, sy);
        float ny11 = lerpp(nx110, nx111, sy);

        float nz0 = lerpp(ny00, ny01, sz);
        float nz1 = lerpp(ny10, ny11, sz);

        

        return lerpp(nz0, nz1, sw);
    }
    
    float eval(float x, float y)
    {
 //       cout << "(" << x << "," << y << ")" << endl;
        double c = 0.5, a = 0.3; // torus parameters (controlling size)
